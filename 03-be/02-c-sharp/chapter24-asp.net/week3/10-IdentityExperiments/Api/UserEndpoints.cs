...
public static class UserEndpoints
{
    public static void MapUserEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (IUserService userService) =>
        {
            ...
            var userDtos = users.Select(u => new UserResponseDto(u.Id, u.FirstName, u.LastName, u.Email!, u.CreatedAt));
        ...
        {
            var user = await userService.CreateAsync(createUserDto.FirstName, createUserDto.LastName, createUserDto.Email);
            var userDto = new UserResponseDto(user.Id, user.FirstName, user.LastName, user.Email!, user.CreatedAt);
        ...
        {
            var user = await userService.UpdateAsync(id, updateUserDto.FirstName, updateUserDto.LastName, updateUserDto.Email);
            if (user == null)
                return Results.NotFound();
        ...
        group.MapGet("/{id:guid}/posts", async (Guid id, IUserService userService, IPostService postService) =>
        {
            ...
            var postDtos = posts.Select(p => new TravelApi.Dtos.Posts.PostResponseDto(p.Id, p.UserId, p.Title, p.Content, p.Image, p.PublishedAt));
            return Results.Ok(postDtos);
        })
        .Produces<IEnumerable<TravelApi.Dtos.Posts.PostResponseDto>>()
        .ProducesProblem(StatusCodes.Status404NotFound);
    }
}